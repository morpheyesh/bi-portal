...
PORTAL.nodes = function() {
	...
	function refreshValidation() {
		for (var n in nodes) {
			PORTAL.editor.validateNode(nodes[n]);
		}
	}
	...
	function createExportableNodeSet(set) {
		...
		for (var n in set) {
			var node = set[n].n;
			var convertedNode = PORTAL.nodes.convertNode(node);
			for (var d in node._def.defaults) {
				if (node._def.defaults[d].type && node[d] in configNodes) {
					...
					if ((exportable == null || exportable)) {
						if (!(node[d] in exportedConfigNodes)) {
							...
							nns.unshift(PORTAL.nodes.convertNode(confNode));
						}
					} else {
						convertedNode[d] = "";
					}
	...
	function importNodes(newNodesObj, createNewIds) {
		try {
			...
			if (unknownTypes.length > 0) {
				...
				PORTAL.notify("<strong>Imported unrecognised " + type + ":</strong>" + typeList, "error", false, 10000);
				//"DO NOT DEPLOY while in this state.<br/>Either, add missing types to portal, restart and then reload page,<br/>or delete unknown "+n.name+", rewire as required, and then deploy.","error");
			}
			...
			if (defaultWorkspace == null) {
				...
				PORTAL.view.addWorkspace(defaultWorkspace);
			}
			var node_map = {};
			var new_nodes = [];
			var new_links = [];
			...
			for (var i in newNodes) {
				...
				if (n.type !== "workspace" && n.type !== "tab") {
					...
					if (def && def.category == "config") {
						...
						if (createNewIds) {
							node.z = PORTAL.view.getWorkspace();
							node.id = getID();
						} else {
							node.id = n.id;
						...
						addNode(node);
						PORTAL.editor.validateNode(node);
						node_map[n.id] = node;
						new_nodes.push(node);
					}
				}
