  ...
  //  function getCredentialsURL(nodeType, nodeID) {
   ...
   // }
    ...
    function validateNodeProperty(node,definition,property,value) {
        ...
        if (valid && definition[property].type && PORTAL.nodes.getType(definition[property].type) && !("validate" in definition[property])) {
            if (!value || value == "_ADD_") {
                valid = false;
            } else {
                var v = PORTAL.nodes.node(value).valid;
                valid = (v==null || v);
            }
        }
        return valid;
    ...
    function updateNodeProperties(node) {
        ...
        if (node.outputs < node.ports.length) {
            ...
            var removedLinks = [];
            PORTAL.nodes.eachLink(function(l) {
                    if (l.source === node && l.sourcePort >= node.outputs) {
                        removedLinks.push(l);
                    }
            });
            for (var l in removedLinks) {
                PORTAL.nodes.removeLink(removedLinks[l]);
            }
    ...
    /*$( "#dialog" ).dialog({
            ...
            buttons: [
                {
                    ...
                    click: function() {
                        if (editing_node) {
                            var changes = {};
                            var changed = false;
                            var wasDirty = PORTAL.view.dirty();
                                    ...
                                    if (newValue != null) {
                                        if (editing_node[d] != newValue) {
                                            if (editing_node._def.defaults[d].type) {
                                                ...
                                                var configNode = PORTAL.nodes.node(editing_node[d]);
                                                if (configNode) {
                                                    var users = configNode.users;
                                                    users.splice(users.indexOf(editing_node),1);
                                                }
                                                var configNode = PORTAL.nodes.node(newValue);
                                                if (configNode) {
                            ...
                            if (changed) {
                                ...
                                PORTAL.history.push({t:'edit',node:editing_node,changes:changes,links:removedLinks,dirty:wasDirty,changed:wasChanged});
                            }
                            editing_node.dirty = true;
                            validateNode(editing_node);
                            PORTAL.view.redraw();
                        } else if (PORTAL.view.state() == PORTAL.state.EXPORT) {
                            if (/library/.test($( "#dialog" ).dialog("option","title"))) {
                                //TODO: move this to PORTAL.library
                                var flowName = $("#node-input-filename").val();
                                if (!/^\s*$/.test(flowName)) {
                                    $.post('library/flows/'+flowName,$("#node-input-filename").attr('nodes'),function() {
                                            PORTAL.library.loadFlowLibrary();
                                            PORTAL.notify("Saved nodes","success");
                                    });
                                }
                            };
                        } else if (PORTAL.view.state() == PORTAL.state.IMPORT) {
                            PORTAL.view.importNodes($("#node-input-import").val());
                        }
                        $( this ).dialog( "close" );
                    }
                },
            ...
            close: function(e) {
                ...
                if (PORTAL.view.state() != PORTAL.state.IMPORT_DRAGGING) {
                    PORTAL.view.state(PORTAL.state.DEFAULT);
                }
                $( this ).dialog('option','height','auto');
                $( this ).dialog('option','width','500');
                if (editing_node) {
                    PORTAL.sidebar.info.refresh(editing_node);
                }
                PORTAL.sidebar.config.refresh();
                editing_node = null;
            }
    });*/
    ...
    /*function prepareConfigNodeSelect(node,property,type) {
        ...
        var node_def = PORTAL.nodes.getType(type);
        ...
        var label = "";
        var configNode = PORTAL.nodes.node(node[property]);
    ...
    function showEditDialog(node) {
        editing_node = node;
        PORTAL.view.state(PORTAL.state.EDITING);           
    ...
    function showEditConfigNodeDialog(name,type,id) {
        ...
        var configNode = PORTAL.nodes.node(id);
        if (configNode == null) {
            configNode = {
            ...
            if (buttons.length == 2) {
                buttons.unshift({
                        ...
                        click: function() {
                            ...
                            var configNode = PORTAL.nodes.node(configId);
                            var configTypeDef = PORTAL.nodes.getType(configType);

                            if (configTypeDef.ondelete) {
                                configTypeDef.ondelete.call(PORTAL.nodes.node(configId));
                            }
                            PORTAL.nodes.remove(configId);
    ...
    function updateConfigNodeSelect(name,type,value) {
        ...
        PORTAL.nodes.eachConfig(function(config) {
            if (config.type == type) {
                var label = "";
                if (typeof node_def.label == "function") {
    ...
    $( "#node-config-dialog" ).dialog({
            ...
            buttons: [
                {
                    ...
                    click: function() {
                        ...
                        if (configAdding) {
                            ...
                            PORTAL.nodes.add(configNode);
                            updateConfigNodeSelect(configProperty,configType,configNode.id);
                        } else {
                            configNode = PORTAL.nodes.node(configId);
                            for (var d in configTypeDef.defaults) {
                                var input = $("#node-config-input-"+d);
                                configNode[d] = input.val();
                            }
                        ...
                        if (configTypeDef.oneditsave) {
                            configTypeDef.oneditsave.call(PORTAL.nodes.node(configId));
                        }
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        if (configTypeDef.oneditcancel) {
                            ...
                            if (configTypeDef.oneditcancel) {
                                var cn = PORTAL.nodes.node(configId);
                                if (cn) {
            ...
            open: function(e) {
                if (PORTAL.view.state() != PORTAL.state.EDITING) {
                    PORTAL.keyboard.disable();
                }
            },
            close: function(e) {
